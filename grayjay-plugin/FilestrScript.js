...
source.searchPlaylists = function (query) {
    ...
    for (const g of res.albums || []) {
        if (g.name.toLowerCase().indexOf(q) !== -1)
            out.push(groupStub("album", g, ""));
    }
    for (const g of res.artists || []) {
        if (g.name.toLowerCase().indexOf(q) !== -1)
            out.push(groupStub("artist", g, ""));
    }
...
source.getChannelPlaylists = function (url) {
    ...
    const out = [];
    for (const g of res.folders || [])
        out.push(groupStub("folder", g, src));
    for (const g of res.albums || [])
        out.push(groupStub("album", g, src));
    for (const g of res.artists || [])
        out.push(groupStub("artist", g, src));
    return new FilestrPlaylistPager(out);
};
...
function recommendationsFor(hash) {
    if (!hash)
...
function gatewayJson(pathAndQuery) {
...
function fetchIndex(pathAndQuery) {
    return gatewayJson(pathAndQuery);
}
...
// small request.
function fetchRelated(hash) {
    return gatewayJson(`/related?hash=${encodeURIComponent(hash)}`).files || [];
}
...
function fetchGrantedPeers() {
    return gatewayJson("/peers").peers || [];
}
...
// one is opened.
function groupStub(kind, g, source) {
