...
import { Daemon, available, waitUntil } from "./harness/daemon.ts";
...
test("offline peer is reported, not silently dropped", { skip }, async () => {
	...
	try {
		...
		assert.ok((await g.files()).peers?.some((p) => p.reachable === true), "peer not reachable while up");
		...
		const desc = String(source.getChannel(subs[0]).description);
