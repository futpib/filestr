...
test("getHome hides non-media; type filter partitions search; streams serve bytes", { skip }, async () => {
	...
	try {
		const song = randomBytes(131072);
		const movie = randomBytes(131072);
		...
		assert.equal(home.results.length, 2, `only the 2 media files, got ${names}`);
		...
		assert.equal(audio + video, all, "audio+video partitions should sum to all");

		// every home item hands back a /file/ stream url that serves the real bytes
		for (const item of home.results) {
			assert.match(String(item.url), /\/file\//, "home item not playable");
			const got = Buffer.from(await (await fetch(String(item.url))).arrayBuffer());
			const want = String(item.name).includes("song") ? song : movie;
