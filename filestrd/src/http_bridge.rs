...
async fn route(state: Arc<State>, req: Request<hyper::body::Incoming>) -> Result<Response<Body>> {
    if req.method() != Method::GET {
        return Ok(text(StatusCode::METHOD_NOT_ALLOWED, "GET only".into()));
    }
    ...
    if let Some(hash) = path.strip_prefix("/file/") {
        ...
        let name = req
            ...
            .unwrap_or_default();
        return serve_file(&state, hash, &name, range.as_deref()).await;
    }
...
/// can fetch it.
async fn list_files(state: &Arc<State>) -> Result<Response<Body>> {
...
async fn serve_file(
    ...
    let size = match state.store.blobs().status(parsed).await? {
    ...
    // chunked stream from the store, clipped to [start, end_excl)
